...
export default function EditCityPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Fetch city
        const cityRes = await fetch(`/api/admin/cities/${resolvedParams.id}`);
        ...
        setFormData({
          ...
          keywords_en: city.keywords_en?.join(', ') || '',
          keywords_ar: city.keywords_ar?.join(', ') || '',
          ...
          priority: city.priority?.toString() || '50',
        });
      } catch (err: any) {
  ...
  const handleDelete = async () => {
    if (!confirm('Are you sure you want to delete this city? This action cannot be undone and will fail if the city has hospitals, treatments, or articles.')) {
      return;
    }
